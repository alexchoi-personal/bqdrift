...
fn create_stored_state_for_query(
    ...
    create_stored_state_with_version(
        query_name,
        partition_date,
        1,
        None,
...
fn test_drift_report_summary_with_mixed_states() {
    ...
    let stored_current = create_stored_state_for_query(
        "simple_query",
        date1,
        current_sql,
        yaml_content,
    ...
    let stored_changed = create_stored_state_for_query(
        "simple_query",
        date2,
        "SELECT 'old' FROM x",
        yaml_content,
...
fn test_immutability_check_no_violation_with_fixtures() {
    ...
    let stored_states = vec![create_stored_state_for_query(
        "simple_query",
        date,
        current_sql,
        yaml_content,
...
fn test_immutability_check_violation_when_source_modified() {
    ...
    let original_executed_sql =
        "SELECT 'was_different' as marker FROM source WHERE date = @partition_date";

    let stored_states = vec![create_stored_state_for_query(
        "simple_query",
        date,
        original_executed_sql,
        yaml_content,
        &version.schema,
    )];
...
fn test_immutability_check_versioned_query_v1_unchanged() {
    ...
    let versioned_query = queries
        .iter()
        .find(|q| q.name == "versioned_query")
        .unwrap();
    ...
    let stored_states = vec![create_stored_state_with_version(
        "versioned_query",
        date,
        1,
        None,
...
fn test_immutability_report_total_affected_partitions() {
    ...
    let version = simple_query
        .get_version_for_date(NaiveDate::from_ymd_opt(2024, 6, 15).unwrap())
        .unwrap();
    ...
    let stored_states: Vec<PartitionState> = dates
        .iter()
        .map(|&date| {
            create_stored_state_for_query(
                "simple_query",
...
fn test_immutability_check_state_without_executed_sql_skipped() {
    ...
    let mut stored_state = create_stored_state_for_query(
        "simple_query",
        date,
        "SELECT 'different'",
        yaml_content,
